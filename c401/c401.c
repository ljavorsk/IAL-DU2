...
**/
	...
	if (*RootPtr == NULL)
	{
		tBSTNodePtr new_root = (tBSTNodePtr) malloc(sizeof(struct tBSTNode));
		if (new_root == NULL)
		{
			return;
		}
		
		new_root->BSTNodeCont = Content;
		new_root->Key = K;
		new_root->LPtr = NULL;
		new_root->RPtr = NULL;
		*RootPtr = new_root;
...
**/
	...
	{
		tBSTNodePtr to_delete;

		if ((*RootPtr)->RPtr == NULL)	// Without right son
		{
			to_delete = *RootPtr;
			if ((*RootPtr)->LPtr == NULL) // Without left son
			{
				// Without sons
				*RootPtr = NULL;
			}
